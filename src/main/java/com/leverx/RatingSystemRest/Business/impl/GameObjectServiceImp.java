...
/**
 * Service which implement GameObjectService interface.
 */
...
public class GameObjectServiceImp implements GameObjectService {
  ...
  public ResponseEntity<String> add(addGameObjectDto dto, MultipartFile photo, int userId) {
    var currentUser = userRepository.findById(userId).orElseThrow(() -> new ResponseStatusException(BAD_REQUEST, SELLER_NOT_FOUND));
    ...
    var gameObject = GameObject.builder().price(dto.getPrice()).title(dto.getTitle()).text(dto.getText()).user(currentUser).created_at(LocalDateTime.now()).picture(savedPicture).build();
  ...
  public ResponseEntity<String> remove(int gameObjectId, int userId) {
    ...
    var getcurrentUser = userRepository.findById(userId).orElseThrow(() -> new ResponseStatusException(BAD_REQUEST, SELLER_NOT_FOUND));

    var currentObject = gameObjectRepository.findById(gameObjectId).orElseThrow(() -> new ResponseStatusException(BAD_REQUEST, GAME_NOT_FOUND));
  ...
  public ResponseEntity<String> update(UpdateGameObject dto, MultipartFile photo, int userId) {
    ...
    var currentObject = gameObjectRepository.findById(dto.getId()).orElseThrow(() -> new ResponseStatusException(BAD_REQUEST, GAME_NOT_FOUND));
  ...
  private GameObjectPicture saveNewPictureOnLocalFolder(int userid, MultipartFile picture) {
    ...
    try {
      ...
      return GameObjectPicture.builder().Url(publicUrl).size(picture.getSize()).Extension(picture.getContentType()).photoName(modifedFileName).build();
  ...
  public ResponseEntity<List<GameObjectDto>> getGameObjectsBySellerId(int sellerId) throws Exception {
    ...
    var getUser = userRepository.findById(sellerId).orElseThrow(() -> new ResponseStatusException(BAD_REQUEST, SELLER_NOT_FOUND));
